    ...
    try:
        for idx, segment_file in enumerate(segment_files):
            ...
            if file_size > 25 * 1024 * 1024:
                raise Exception(f"Segment {idx} too large ({file_size / 1024 / 1024:.2f}MB). Max 25MB.")
...
def health():
    return jsonify({
        ...
        'platform': 'railway',
        'version': '2.0.0',
        'features': ['file_upload', 'audio_processing', 'ai_transcription', 'health_check'],
        'message': 'Audio processing and AI transcription enabled!'
    })
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port, debug=False)def segment_audio(job_dir, audio_file, segment_duration=600):
    """Split audio into segments using FFmpeg"""
    segments_dir = os.path.join(job_dir, 'segments')
    os.makedirs(segments_dir, exist_ok=True)
