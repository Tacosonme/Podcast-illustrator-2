...
# Load Faster Whisper model (using 'base' for balance of speed and accuracy)
whisper_model = None
try:
    ...
    print("Application will start but transcription will not work")
    import traceback
    traceback.print_exc()
...
def transcribe_audio_segments(job_dir, segment_files):
    """Transcribe audio segments using Faster-Whisper model"""
    
    # Check if model is loaded
    if whisper_model is None:
        raise Exception("Whisper model not loaded. Cannot transcribe audio.")
    
    transcripts_dir = os.path.join(job_dir, 'transcripts')
    ...
    try:
        for idx, segment_file in enumerate(segment_files):
            # Update progress
            progress = 30 + int((idx / total_segments) * 40)  # 30-70% for transcription
            update_job_status(job_dir, 'processing', progress, 
                            f'Transcribing segment {idx + 1}/{total_segments}...')
            
            # Transcribe using Faster-Whisper model
            segments, info = whisper_model.transcribe(segment_file, beam_size=5)
            ...
            transcript_file = os.path.join(transcripts_dir, f'transcript_{idx:03d}.json')
        ...
        combined_text = '\n\n'.join([t['text'] for t in all_transcripts])
        combined_file = os.path.join(job_dir, 'full_transcript.txt')
        with open(combined_file, 'w') as f:
            f.write(combined_text)
        
        # Save detailed transcript data
        detailed_file = os.path.join(job_dir, 'transcript_data.json')
        ...
        update_job_status(job_dir, 'processing', 70, 
                        f'Transcription complete! {total_segments} segments processed.')
        
        return all_transcripts, combined_text
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge { background: #28a745; color: white; padding: 5px 10px; border-radius: 5px; font-size: 0.8em; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <p><span class="badge">✨ 100% FREE - Faster-Whisper</span></p>
        
        <!-- Debug test button -->
        <button onclick="alert('JavaScript is working!')" style="margin: 10px; padding: 5px 10px; font-size: 12px;">Test JavaScript</button>
        ...
        <div class="features">
            ...
            <div class="feature">
                ...
                <p>Faster-Whisper model - completely free!</p>
            </div>
            <div class="feature">
        ...
        <div class="upload-area" id="uploadArea">
            ...
            <p>Select your audio file and click Upload & Process</p>
            <div style="margin: 20px 0;">
                <input type="file" id="audioFile" accept="audio/*" style="display: block; margin: 10px auto;" />
            </div>
            <button onclick="uploadFile()" id="uploadBtn" style="display: block; margin: 10px auto;">Upload & Process</button>
        </div>
    ...
    <script>
        ...
        async function uploadFile() {
            ...
            alert('Got elements: ' + (fileInput ? 'YES' : 'NO')); // Debug test
            
            if (!fileInput.files[0]) {
                alert('No file selected'); // Debug test
                showResult('result', 'Please select an audio file first.', 'error');
                return;
            }
            
            alert('File selected: ' + fileInput.files[0].name); // Debug test
            ...
            try {
                ...
                if (response.ok) {
                    showResult('result', `
                        ...
                        <em>Note: Local transcription may take a few minutes...</em>
                    `, 'success');
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (data.status === 'completed') {
                            ...
                            try {
                                const transcriptResponse = await fetch(`${API_BASE}/api/transcript/${jobId}`);
                                ...
                                if (transcriptResponse.ok) {
                                    showResult('statusResult', `
                                        ...
                                        <div style="max-height: 300px; overflow-y: auto; background: #f8f9fa; padding: 10px; border-radius: 5px; margin-top: 10px; text-align: left;">
                                            ${transcriptData.full_text.replace(/\n/g, '<br>')}
                                        </div>
                                        <br>
                                        <button onclick="downloadTranscript('${jobId}')">Download Transcript</button>
                                    `, 'success');
                                } else {
                                    showResult('statusResult', `
                                        <strong>✅ Processing Complete!</strong><br>
        ...
        async function downloadTranscript(jobId) {
            try {
                const response = await fetch(`${API_BASE}/api/transcript/${jobId}`);
                const data = await response.json();
...
def health():
    return jsonify({
        ...
        'platform': 'railway',
        'version': '2.0.0-free',
        'features': ['file_upload', 'audio_processing', 'faster_whisper_transcription', 'health_check'],
        'message': 'Audio processing and FREE Faster-Whisper transcription enabled!'
    })
...
def process_audio(job_id):
    ...
    try:
        ...
        # Start processing in background using subprocess
        import threading
        ...
        def process_in_background():
            try:
                ...
                transcripts, full_text = transcribe_audio_segments(job_dir, segments)
                
                # Mark as completed
                update_job_status(job_dir, 'completed', 100, 
                                f'Processing complete! {len(segments)} segments transcribed.')
            except Exception as e:
...
def get_transcript(job_id):
    ...
    job_dir = os.path.join(app.config['UPLOAD_FOLDER'], job_id)
    transcript_file = os.path.join(job_dir, 'full_transcript.txt')
    transcript_data_file = os.path.join(job_dir, 'transcript_data.json')
    ...
    try:
        ...
        # Read detailed transcript data if available
        transcript_data = None
        if os.path.exists(transcript_data_file):
